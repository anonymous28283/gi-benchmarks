...
describe("uint53 check", function()
	...
	it("other types & non-uint numbers", function()
		assert.equal(false, is_uint53("str"))
		assert.equal(false, is_uint53({ x = 42 }))
		assert.equal(false, is_uint53(-42))
		assert.equal(false, is_uint53(42.5))
		assert.equal(false, is_uint53(2 ^ 53))
	end)
