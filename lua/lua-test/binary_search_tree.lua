...
local function succ_node(self, key)
	...
	local function succ(node)
		...
		if less_than(key, node.key) then
			return nilor(succ(node[true]), node)
		end
		if less_than(node.key, key) then
			return succ(node[false])
		end
		return min_node(node[false])
	end
