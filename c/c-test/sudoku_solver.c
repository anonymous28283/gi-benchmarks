...
bool solve(struct sudoku *a)
{
    ...
    for (uint8_t v = 1; v <= a->N; v++)
    {
        ...
        if (OK(a, i, j, v))
        {
            ...
            printf("%sBacktrack (%d,%d) <- %d (counter=%d)\n",
...
int main()
{
    ...
    for (int i = 0; i < a.N; i++)
        for (int j = 0; j < a.N; j++) scanf("%d", &(a.a[i * a.N + j]));

    printf("Entered a %udx%ud matrix with block size: %d\n", a.N, a.N,
           a.N2);
